...
2. **重复登记同一 URL 会被拒，已做幂等。**
   上游按 `inputUrl` 去重，同一个 URL 第二次 ImportMedia 返回
   `400 MediaAlreadyExist`，而**不是**回那条已有素材。
   `CreateAsset` 捕获这个错误码，从错误文案里取出已有的 mediaId 当成功返回。

   解析错误文案是下策，但这是唯一的信息来源 —— 上游没有「按 url 查 media」的接口。
   兜底方向是安全的：抠不到就照常抛原错误，也就是退回改之前的行为。
   上游改文案的代价是功能退化，不是拿到一个错的 id 去引用。
   `TestYikeMediaIDInError` 的第一条用例是生产实测原文，不是照文档臆造的。
